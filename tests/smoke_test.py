...
def test_ram_stage_single_source_v1080() -> None:
    """v10.8.0：拉姆阶段单一真源——RamAI 绑定 engine 时以引擎为唯一计算源。"""
    from shared.state import HardStateEngine, RamStage
    from shared.prompts import RamAI

    # 1. 绑定 engine 时 stage() == engine._get_ram_stage()
    engine = HardStateEngine()
    ram = RamAI(engine=engine)
    assert ram.stage() == engine._get_ram_stage(), "绑定初始阶段应一致"

    # 2. 只改 engine.ram_favor，不调用 _update_stage() 也一致
    engine.ram_favor = 50
    assert ram.stage() == engine._get_ram_stage() == RamStage.DECENT, \
        f"直接改 ram_favor 后应一致，实际 {ram.stage()} vs {engine._get_ram_stage()}"

    # 3. 阶段跃迁覆盖多档
    for fav, expected in [(25, RamStage.OBSERVING), (46, RamStage.DECENT),
                          (66, RamStage.RELUCTANT), (85, RamStage.ACKNOWLEDGED)]:
        engine.ram_favor = fav
        assert ram.stage() == expected, f"ram_favor={fav} 应为 {expected.value}，实际 {ram.stage().value}"

    # 4. 未绑定 engine 的旧路径仍可用
    solo = RamAI()
    solo.on_rem_treated_well(80)  # favor 8+80=88 → ACKNOWLEDGED
    assert solo.favor() == 88, f"未绑定 favor 应为 88，实际 {solo.favor()}"
    assert solo.stage() == RamStage.ACKNOWLEDGED, "未绑定 _update_stage 应仍工作"

    # 5. 本地模式集成一致性
    twin = ReZeroTwinSystem()
    twin.ram.on_rem_treated_well(80)  # favor 8+80=88
    assert twin.ram.stage() == twin.rem.engine._get_ram_stage(), "本地模式阶段应与引擎一致"
    assert twin.ram.stage() == RamStage.ACKNOWLEDGED
...
def main() -> int:
    tests = [
        ...
        ("拉姆阶段单一真源 v10.8.0", test_ram_stage_single_source_v1080),
