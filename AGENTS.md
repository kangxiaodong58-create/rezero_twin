...
架构审批强制整改项 **M1–M7 全部已落地并闭环**（M7 = 门禁真实生效：**L2 本地等价留证 + L3 远端首跑转绿**；2026-09-23 首次批量搬运时首跑/复跑两度红 → 逐轮修复（取证 annotations 化、Qt 夹具可移植化、截图基线尺寸不对称 A20）→ **第 3 跑 `#35800786078` success**；判据与豁免边界见 `docs/specs/INDEX.md`「M7 闭环判据与豁免条款」段，全程留证 `docs/devlog/L3远端首跑留证_2026-09-23.md`）。
