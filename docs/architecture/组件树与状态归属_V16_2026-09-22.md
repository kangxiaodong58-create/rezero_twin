...
|---|---|---|---|---|---|
| **B0** | 删 `test_mode.py`（**A16**）· 顶栏死 UI 标注（**A17**） | L0 | — | `git status` 无该文件；文档登记 | 否 |
| **B1** | 删 `self.mem` 陈旧读 2 处（**A13**） | L1 | — | 全量 295 passed；重启恢复一致 | 否 |
| **B2** | 世界行统一格式化（**A15**）· 删 `/toggle` 死命令与 `mode` 字段（**A3** 实例）· 拉姆表情映射入 token（**D3** 实例） | L1 ×3 | — | 命令路由回归（`test_command_router.py`）+ 截图基线 | 否（文案不变） |
