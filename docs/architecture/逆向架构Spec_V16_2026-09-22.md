...
| 批次 | 项 | 状态 |
|---|---|---|
| **V16.1**（2026-09-22） | M1 原子写 + 损坏不静默 · M2 引擎状态单源序列化 · A3 死状态清理 · A5 隔离根因（含施工中新发现的 A7） | ✅ **已落地**（`276/276` 通过；`data/` 逐字节未变实测） |
| V16.2（第 2 批） | M3 两阶段提交 `TurnTxn` · M4 失败路径测试 · M5 探针剩余固化 | 待施工（决策：world 变更保留即时，仅 `scene_first` 记账挪到 commit） |
