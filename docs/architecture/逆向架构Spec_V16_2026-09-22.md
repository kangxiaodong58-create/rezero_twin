...
**A21 [P3] 顶栏「设置」入口绑定 `/toggle` 死命令（SPEC-12 只读侦察发现）**
- 事实：顶栏 nav「设置」按钮回调 = `self._handle_command("/toggle")`；而 `/toggle` 自 V14.4（本地模式下线）起**仅提示、无实际切换**（只记日志 + 一条系统消息）。即「设置」点下去什么都不设置。
- 后果：死入口（用户预期落空）；但**不能直接删** `/toggle` —— 删了该入口会回落到「未知指令」系统提示（更差）。
- 修法（待设计）：先给「设置」定真实落点（设置面板 / 对话框 / `/help` 列表），再删 `/toggle`；属**交互变更 → 需真机确认门禁**。
- **状态：⏳ 未修（V16.4.0 记录；`mode` 字段已随 SPEC-20260922-12 删除，命令入口本身保留）**
