...
| **V16.3.2**（2026-09-22） | CI 首跑修红：删除误设的 `REZERO_DISABLE_UI_MOTION=1`（致 2 例 test_motion 失败）· 测试/隔离两步拆分；**A11/A12 登记** | ⚠️ **部分**：ci.yml 修完待第二次 CI 运行验证；A11/A12 修复需 RCR（测试文件范围） |
...
- **状态：⏳ 未修**（需要 RCR：涉及 `tests/` 未列入 SPEC 的文件）
...
- **状态：⏳ 未修**（同属测试文件范围，随 A11 一并走 RCR）。**操作规避**（未修期间）：自己跑冒烟时前置 `REZERO_GUI_LOG=$TMPDIR/smoke.log`。
